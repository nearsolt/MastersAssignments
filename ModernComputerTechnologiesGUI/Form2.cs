...
namespace ModernComputerTechnologiesGUI {
    public partial class Form2 : Form {
        ...
        private Complex[] arrayOfFuncValuesComplex;
        #endregion
        ...
        private void BuildChartOfOriginalRealFunc() {
            ...
            Helper.CalcArrayOfValuesX(arrayOfValuesX, numberN, startOfInterval, endOfInterval);
            arrayOfFuncValues = new double[numberN];
            Helper.CalcArrayOfFuncValues(arrayOfValuesX, arrayOfFuncValues, numberN);
            for (int j = 0; j < numberN ; j++) {
        ...
        private void BuildChartOfModifiedRealFunc() {
            ...
            numberN = (int)this.numericUpDown_numN.Value;
            zeroingPercentage = (int)this.numericUpDown_percent.Value;
            arrayOfFuncValuesComplex = new Complex[numberN];
            for (int j = 0; j < numberN; j++) {
                arrayOfFuncValuesComplex[j] = (Complex)arrayOfFuncValues[j];
            }
            Complex[] arrayCoeffC = Helper.DFT((Complex[])arrayOfFuncValuesComplex.Clone(), numberN);
            int numberOfZeroed = numberN * zeroingPercentage / 100;
            #region case 3
            Tuple<Complex, int>[] tupleArrayCoeffC = new Tuple<Complex, int>[numberN];
            for (int j = 0; j < numberN; j++) {
            ...
            #endregion
            Complex[] arrayOfModComplexFunc = Helper.DFT((Complex[])arrayCoeffC.Clone(), numberN, true);
            Complex[] sortedArrayOfModComplexFunc = ((Complex[])arrayOfModComplexFunc.Clone()).OrderBy(c => c.Real).ToArray();
            double[] arrayOfFuncValuesQQQQ = new double[numberN];
            for (int j = 0; j < numberN; j++) {
                arrayOfFuncValuesQQQQ[j]= sortedArrayOfModComplexFunc[j].Real;
            }
            for (int j = 0; j < numberN; j++) {
                this.chart_mainChart.Series[1].Points.AddXY(arrayOfValuesX[j], arrayOfFuncValuesQQQQ[j]);
            }
            #region debug BuildChartOfModifiedComplexFunc
            ...
            if (this.checkBox_debug.Checked) {
                string info2 = string.Empty;
                for (int j = 0; j < numberN; j++) {
                    info2 += $"({arrayOfValuesX[j]}, {arrayOfFuncValuesQQQQ[j]}), ";
                }
                this.label_infoString2.Text = $"(x_k, f_k^):\n{info2}";

                foreach (Complex item in arrayCoeffC) {
                    Console.Write($"({item.Real}, {item.Imaginary}), ");
                }
                Console.WriteLine($" - array coeff");
                foreach (Complex item in sortedArrayOfModComplexFunc) {
                    Console.Write($"({item.Real}, {item.Imaginary}), ");
                }
                Console.WriteLine($" - sorted array func");
            }
            #endregion
